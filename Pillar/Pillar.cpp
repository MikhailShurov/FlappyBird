...
Pillar::Pillar() : birdPass_(false), incrementEmited_(false){
    ...
    topPillar->setPos(
            QPointF(0, 0) - QPointF(topPillar->boundingRect().width() / 2, topPillar->boundingRect().height() + 80));
    bottomPillar_->setPos(QPointF(0, 0) - QPointF(bottomPillar_->boundingRect().width() / 2, -80));
    ...
    y_ = QRandomGenerator::global()->bounded(-200, 200);
//    int startX = QRandomGenerator::global()->bounded(100);
    ...
    xAnimation_->setEndValue(-1900);
    xAnimation_->setEasingCurve(QEasingCurve::Linear);
    xAnimation_->setDuration(8500);
    xAnimation_->start();
...
//    }
    if (this->scenePos().x() < 0 && !incrementEmited_) {
        incrementEmited_ = true;
//        qDebug() << "emited";
        emit(incrementScore());
    }
}
